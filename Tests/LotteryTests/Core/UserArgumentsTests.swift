...
final class UserArgumentsTests: XCTestCase {
    ...
    func test_init_throwsMinimumNumberError() {
        ...
        XCTAssertThrowsError(try UserArguments(arguments: [
            "--maximumNumber",
            "49",
            "--count",
            "6"
        ...
        XCTAssertEqual(
            thrownError as! UserArgumentsError,
            .minimumNumberError
        )
     }
    ...
    func test_init_throwsCountError() {
     ...
     func test_init_fromStandardArgumentsOrder() throws {
         let userArguments = try UserArguments(arguments: [
            "--minimumNumber",
            "1",
            "--maximumNumber",
            "49",
         ...
         XCTAssertEqual(
            1,
            userArguments.minimumNumberValue
         )
         XCTAssertEqual(
            49,
            userArguments.maximumNumberValue
         )
         XCTAssertEqual(
            6,
            userArguments.countValue
         )
     }
    
    func test_init_fromDifferentArgumentsOrder() throws {
        let userArguments = try UserArguments(arguments: [
            "--count",
            "6",
            "--maximumNumber",
