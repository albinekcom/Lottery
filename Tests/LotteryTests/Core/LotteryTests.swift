...
final class LotteryTests: XCTestCase {
    ...
    func test_results_throwsMinimumNumberIsGreaterThanMaximumNumberError() {
        ...
        XCTAssertThrowsError(try Lottery().results(
            minimumNumber: 60,
    ...
    func test_results_areGreaterOrEqualMinimum() throws {
        let results = try Lottery().results(
            minimumNumber: defaultMinimumNumber,
            maximumNumber: defaultMaximumNumber,
            count: defaultCount
        )
        ...
        results.forEach {
            XCTAssertGreaterThanOrEqual(
                $0,
                defaultMinimumNumber
            )
        }
    }
    
    func test_results_areLessOrEqualMaximum() throws {
        let results = try Lottery().results(
            minimumNumber: defaultMinimumNumber,
            maximumNumber: defaultMaximumNumber,
            count: defaultCount
        ...
        results.forEach {
            XCTAssertLessThanOrEqual(
                $0,
                defaultMaximumNumber
            )
        }
    }
    
    func test_reults_countAreTheSameAsProvided() throws {
        let results = try Lottery().results(
            minimumNumber: defaultMinimumNumber,
            maximumNumber: defaultMaximumNumber,
            count: defaultCount
    ...
    func test_results_areShuffled() throws {
    ...
    func test_reults_areSorted() throws {
        let results = try Lottery().results(
            minimumNumber: defaultMinimumNumber,
            maximumNumber: defaultMaximumNumber,
            count: defaultCount
    ...
    func test_lotteryErrors_description() {
        XCTAssertEqual(
            "Argument \"Minimum number\" is greater than argument \"Maximum number\"",
            LotteryError.minimumNumberIsGreaterThanMaximumNumber.description
        )
        XCTAssertEqual(
            "Argument \"Count\" is greater than all numbers count",
